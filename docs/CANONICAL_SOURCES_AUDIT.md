...
| **A4** | Replace `assert_join()` with `mysterycall_safe_*_join()` / `mysterycall_assert_unique_keys()`. | `private_equity` | in progress |
...
| **A7** | Regenerate the STROBE figure with `mysterycall_strobe_flow()` instead of the hand-written `strobe_diagram.R`, which still reports a superseded cohort of 544. | `private_equity` | open |
